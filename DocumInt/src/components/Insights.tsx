  ...
  const buildFrontExpanded = useCallback((selected: string, analysis: GeminiAnalysisResp) => {
    ...
    return (
      <div className="space-y-4">
        ...
        {top.length>0 && <div><div className="text-xs font-semibold text-gray-700 mb-1">Top Points</div><div className="text-xs space-y-1 prose prose-xs max-w-none">{top.map(t=> <div key={t.slice(0,50)} className="mb-1"><ReactMarkdown>{t}</ReactMarkdown></div>)}</div></div>}
        ...
        {details.length>0 && <div className="space-y-2"><div className="text-xs font-semibold text-gray-700">Details</div>{details.slice(0,3).map((d,i)=>{ const key=`det_${i}`; return <div key={key} className="border rounded p-2 bg-gray-50"><div className="text-[11px] text-gray-500 mb-1">{d.document} • {d.section_title} (p.{d.page_number})</div><div className="text-xs leading-snug prose prose-xs max-w-none"><ReactMarkdown>{(d.gemini_analysis||'').slice(0,800)}</ReactMarkdown></div></div>; })}</div>}
