                    ...
                    <div ref={canvasRef} className="relative flex-1 z-[1] select-none">
                        ...
                        {(() => { const p = ensurePanels(); return (
                            <>
                                ...
                                {/* Overlays independent of layout */}
                                <SelectionBulb apis={isAdobeLoaded ? window.AdobeDC : null} onGenerateInsight={(selectedText: string) => console.log('Generating insight for:', selectedText)} />
                                <Chat chatHistory={chatHistory} chatMessage={chatMessage} pdfFiles={pdfList} projectName={projectName} onMessageChange={setChatMessage} onSendMessage={(message: string, results?: any[]) => { if (message.trim()) { const newMessage = { id: crypto.randomUUID(), type: 'user' as const, message, timestamp: new Date(), results }; setChatHistory(prev => [...prev, newMessage]); setChatMessage(''); } }} onNavigateToPage={handlePageNavigation} />
                            </>
                        )})()}
                    </div>
